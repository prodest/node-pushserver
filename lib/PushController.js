...
let pushAssociations = require( './PushAssociations' );
// let apnPusher = require( './APNPusher' );
let gcmPusher = require( './GCMPusher' );
...
var send = function( pushAssociations, androidPayload, iosPayload ) {
    ...
    if ( androidPayload && androidTokens.length > 0 ) {
        let gcmPayload = gcmPusher.buildPayload( androidPayload );
        gcmPusher.push( androidTokens, gcmPayload );
    }

    if ( iosPayload && iosTokens.length > 0 ) {
        let gcmPayload = gcmPusher.buildPayload( iosPayload );
        gcmPusher.push( iosTokens, gcmPayload );
    }
};
...
var subscribe = function( deviceInfo ) {
    pushAssociations.update( deviceInfo.user, deviceInfo.type, deviceInfo.token, deviceInfo.sub );
};
