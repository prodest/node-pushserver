...
app.post( '/send-android-ios', authentication.ensureAuthenticated, function( req, res ) {
    const pushData = [ createPushObject( req.body.users, req.body.message, req.body.state, req.body.stateParams, req.body.icon ) ];
...
function createPushObject( users, message, state, stateParams, icon ) {
    if ( !icon ) {
        icon = 'notification';
    }
